...
function Rebuild-MaterialByOrder {
    ...
    $seqArr = @($Sequence)
    if ($null -eq $Units -or @($Units.Keys).Count -eq 0) {
        return @{ text = ""; ok = $false; reason = "no_units" }
    }
    if ($null -eq $Sequence -or $seqArr.Count -eq 0) {
        return @{ text = ""; ok = $false; reason = "no_sequence" }
    }
    $parts = New-Object System.Collections.Generic.List[string]
    foreach ($id in $seqArr) {
        if (-not $Units.ContainsKey($id)) {
            return @{ text = ""; ok = $false; reason = "sequence_id_missing:$id" }
        }
...
$subfamilyName = U @(0x786E,0x5B9A,0x6346,0x7ED1)

$nameOther = U @(0x5176,0x4ED6,0xFF08,0x786E,0x5B9A,0x6346,0x7ED1,0xFF09)
$nameRelOther = U @(0x5173,0x8054,0x8BCD,0x002D,0x5176,0x4ED6)
$nameRelParallel = U @(0x5173,0x8054,0x8BCD,0x002D,0x5E76,0x5217)
$nameRelTurning = U @(0x5173,0x8054,0x8BCD,0x002D,0x8F6C,0x6298)
$namePronoun = U @(0x6307,0x4EE3,0x8BCD)

$batchName = "sentence_order_fixed_bundle_$BatchDate"
...
$docxFiles = @(Get-ChildItem -LiteralPath $InputDir -Filter "*.docx" | Where-Object {
    ...
    ($base -eq $nameRelOther) -or
    ($base -eq $nameRelParallel) -or
    ($base -eq $nameRelTurning) -or
    ($base -eq $namePronoun)
} | Sort-Object Name)
...
foreach ($f in $docxFiles) {
    ...
    $leafGuess = "fixed_bundle_other"
    if ($patternTag -eq $nameRelOther) { $leafGuess = "rel_other" }
    elseif ($patternTag -eq $nameRelParallel) { $leafGuess = "rel_parallel" }
    elseif ($patternTag -eq $nameRelTurning) { $leafGuess = "rel_turning" }
    elseif ($patternTag -eq $namePronoun) { $leafGuess = "pronoun" }
    elseif ($patternTag -eq $nameOther) { $leafGuess = "other_fixed_bundle" }
    ...
    foreach ($blockRaw in $blocks) {
        ...
        $unitCount = if ($null -eq $units) { 0 } else { $units.Count }
...
$overview = @"
# Distill Pack Overview (sentence_order / fixed_bundle)

## Method
- Build ordered units from prompt text (e.g. 鈶犫憽鈶?..).
