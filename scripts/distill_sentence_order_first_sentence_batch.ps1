...
$subfamilyName = U @(0x786E,0x5B9A,0x9996,0x53E5)

$nameNonRelTail = U @(0x975E,0x9996,0x53E5,0x7279,0x5F81,0x002D,0x5173,0x8054,0x8BCD,0x7684,0x540E,0x534A,0x90E8,0x5206)
$nameNonExample = U @(0x975E,0x9996,0x53E5,0x7279,0x5F81,0x002D,0x4E3E,0x4F8B,0x5B50)
$nameNonOther = U @(0x975E,0x9996,0x53E5,0x7279,0x5F81,0x002D,0x5176,0x4ED6)
$nameNonPronoun = U @(0x975E,0x9996,0x53E5,0x7279,0x5F81,0x002D,0x6307,0x4EE3,0x4E0D,0x6E05,0x7684,0x6307,0x4EE3,0x8BCD)
$nameHeadBackground = U @(0x9996,0x53E5,0x7279,0x5F81,0x002D,0x80CC,0x666F,0x5F15,0x5165)
$nameHeadViewpoint = U @(0x9996,0x53E5,0x7279,0x5F81,0x002D,0x63D0,0x51FA,0x89C2,0x70B9)
$nameHeadDefinition = U @(0x9996,0x53E5,0x7279,0x5F81,0x002D,0x4E0B,0x5B9A,0x4E49)

$batchName = "sentence_order_first_sentence_$BatchDate"
...
$docxFiles = @(Get-ChildItem -LiteralPath $InputDir -Filter "*.docx" | Where-Object {
    ...
    ($base -eq $nameNonRelTail) -or
    ($base -eq $nameNonExample) -or
    ($base -eq $nameNonOther) -or
    ($base -eq $nameNonPronoun) -or
    ($base -eq $nameHeadBackground) -or
    ($base -eq $nameHeadViewpoint) -or
    ($base -eq $nameHeadDefinition)
} | Sort-Object Name)
...
foreach ($f in $docxFiles) {
    ...
    $leafGuess = "first_sentence_other"
    if ($patternTag -eq $nameNonRelTail) { $leafGuess = "nonfirst_rel_tail" }
    elseif ($patternTag -eq $nameNonExample) { $leafGuess = "nonfirst_example" }
    elseif ($patternTag -eq $nameNonOther) { $leafGuess = "nonfirst_other" }
    elseif ($patternTag -eq $nameNonPronoun) { $leafGuess = "nonfirst_pronoun_ambiguous" }
    elseif ($patternTag -eq $nameHeadBackground) { $leafGuess = "first_background_intro" }
    elseif ($patternTag -eq $nameHeadViewpoint) { $leafGuess = "first_raise_viewpoint" }
    elseif ($patternTag -eq $nameHeadDefinition) { $leafGuess = "first_definition" }
...
$overview = @"
# Distill Pack Overview (sentence_order / first_sentence)
