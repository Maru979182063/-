...
$subfamilyName = U @(0x6A2A,0x7EBF,0x5728,0x5F00,0x5934)

$nameHeadSummary = U @(0x6A2A,0x7EBF,0x5728,0x5F00,0x5934,0x002D,0x6982,0x62EC,0x540E,0x6587)
$nameHeadClause = U @(0x6A2A,0x7EBF,0x5728,0x5F00,0x5934,0x002D,0x6A2A,0x7EBF,0x4E3A,0x9996,0x53E5,0x4E2D,0x7684,0x5206,0x53E5)
$nameHeadTopic = U @(0x6A2A,0x7EBF,0x5728,0x5F00,0x5934,0x002D,0x8BDD,0x9898,0x5F15,0x5165)

$batchName = "sentence_fill_head_start_$BatchDate"
...
$docxFiles = Get-ChildItem -LiteralPath $InputDir -Filter "*.docx" | Where-Object {
    ...
    ($base -eq $nameHeadSummary) -or ($base -eq $nameHeadClause) -or ($base -eq $nameHeadTopic)
} | Sort-Object Name
...
foreach ($f in $docxFiles) {
    ...
    $leafGuess = 'head_other'
    if ($patternTag -eq $nameHeadSummary) { $leafGuess = 'head_summary' }
    elseif ($patternTag -eq $nameHeadClause) { $leafGuess = 'head_clause' }
    elseif ($patternTag -eq $nameHeadTopic) { $leafGuess = 'head_topic_intro' }
...
$summary = [PSCustomObject]@{
    ...
    input_dir = $InputDir
    taxonomy = @{ mother_family = 'sentence_fill'; subfamily = 'head_start' }
    total_samples = $total
...
$overview = @"
# Distill Pack Overview (sentence_fill / head_start)
