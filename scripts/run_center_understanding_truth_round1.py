...
DATE_TAG = "2026-04-14"
RESULTS_PATH = REPORTS_DIR / f"center_understanding_truth_round1_results_{DATE_TAG}.csv"
PACK_PATH = REPORTS_DIR / f"center_understanding_truth_round1_pack_{DATE_TAG}.md"
...
def main() -> None:
    ...
    manifest_rows = [
        ...
        if row.get("family") == "center_understanding"
    ]
    service = build_service()

    result_rows: list[dict[str, str]] = []
    pack_sections: list[str] = ["# center_understanding Truth Rewrite Round 1", ""]
    ...
    for manifest_row in manifest_rows:
        ...
        source_question = parse_source_question(block["lines"], family="center_understanding") if block else {
            "passage": "",
            "stem": "",
            "options": {},
            "answer": "",
...
def _run_single_case(
    *,
    ...
    center_axis_candidate_text = ""
    center_support_excerpt = ""
    ...
    try:
        ...
        request_snapshot = service._build_request_snapshot(
            ...
            standard_request,
            decoded,
            request_id=f"cu-round1::{uuid4().hex}",
        ...
        prompt_extras = ((prepared_material.source or {}).get("prompt_extras") or {}) if isinstance(prepared_material.source, dict) else {}
        center_axis_candidate_text = str(prompt_extras.get("center_axis_candidate_text") or "").strip()
        center_support_excerpt = str(prompt_extras.get("center_support_excerpt") or "").strip()
        built_item = service._build_generated_item(
            ...
            batch_id=f"cu-round1::{uuid4().hex}",
            item_id=None,
            request_snapshot=request_snapshot,
            revision_count=0,
            ...
            review_note=None,
            request_id=f"cu-round1-item::{uuid4().hex}",
            previous_item=None,
        )
    ...
    return {
        ...
        "center_axis_candidate_text": center_axis_candidate_text,
        "center_support_excerpt": center_support_excerpt,
...
def _render_case_markdown(row: dict[str, str]) -> list[str]:
    ...
    lines = [
        ...
        "",
        "**Center Axis Candidate**",
        "",
        row["center_axis_candidate_text"] or "(empty)",
        "",
        "**Center Support Excerpt**",
        "",
        row["center_support_excerpt"] or "(empty)",
        "",
        "**Anchor Stem**",
        "",
        f"> {row['anchor_stem']}",
