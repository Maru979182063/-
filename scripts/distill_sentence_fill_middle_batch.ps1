...
$subfamilyName = U @(0x6A2A,0x7EBF,0x5728,0x4E2D,0x95F4)

$nameCarryPrev = U @(0x6A2A,0x7EBF,0x5728,0x4E2D,0x95F4,0x002D,0x627F,0x4E0A)
$nameBridge = U @(0x6A2A,0x7EBF,0x5728,0x4E2D,0x95F4,0x002D,0x627F,0x4E0A,0x542F,0x4E0B)
$nameLeadNext = U @(0x6A2A,0x7EBF,0x5728,0x4E2D,0x95F4,0x002D,0x542F,0x4E0B)

$batchName = "sentence_fill_middle_$BatchDate"
...
$docxFiles = Get-ChildItem -LiteralPath $InputDir -Filter "*.docx" | Where-Object {
    ...
    ($base -eq $nameCarryPrev) -or ($base -eq $nameBridge) -or ($base -eq $nameLeadNext)
} | Sort-Object Name
...
foreach ($f in $docxFiles) {
    ...
    $leafGuess = 'middle_other'
    if ($patternTag -eq $nameCarryPrev) { $leafGuess = 'middle_carry_prev' }
    elseif ($patternTag -eq $nameBridge) { $leafGuess = 'middle_bridge' }
    elseif ($patternTag -eq $nameLeadNext) { $leafGuess = 'middle_lead_next' }
...
$summary = [PSCustomObject]@{
    ...
    input_dir = $InputDir
    taxonomy = @{ mother_family = 'sentence_fill'; subfamily = 'middle' }
    total_samples = $total
...
$overview = @"
# Distill Pack Overview (sentence_fill / middle)
