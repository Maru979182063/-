...
DATE_TAG = "2026-04-14"
RESULTS_PATH = REPORTS_DIR / f"sentence_order_truth_round1_results_{DATE_TAG}.csv"
PACK_PATH = REPORTS_DIR / f"sentence_order_truth_round1_pack_{DATE_TAG}.md"
...
def main() -> None:
    ...
    manifest_rows = [
        ...
        if row.get("family") == "sentence_order"
    ]
    service = build_service()

    result_rows: list[dict[str, str]] = []
    pack_sections: list[str] = ["# sentence_order Truth Rewrite Round 1", ""]
    ...
    for manifest_row in manifest_rows:
        ...
        source_question = parse_source_question(block["lines"], family="sentence_order") if block else {
            "passage": "",
            "stem": "",
            "options": {},
            "answer": "",
    ...
    *,
    ...
    try:
        ...
        request_snapshot = service._build_request_snapshot(
            ...
            standard_request,
            decoded,
            request_id=f"so-round1::{uuid4().hex}",
        ...
        prompt_extras = ((prepared_material.source or {}).get("prompt_extras") or {})
        sortable_material_text = str(prompt_extras.get("sortable_material_text") or prepared_material_text).strip()
        binding_pairs_json = json.dumps(prompt_extras.get("binding_pairs") or [], ensure_ascii=False)
        sentence_roles_json = json.dumps(prompt_extras.get("sentence_roles") or {}, ensure_ascii=False)
        built_item = service._build_generated_item(
            ...
            batch_id=f"so-round1::{uuid4().hex}",
            item_id=None,
            request_snapshot=request_snapshot,
            revision_count=0,
            ...
            review_note=None,
            request_id=f"so-round1-item::{uuid4().hex}",
            previous_item=None,
        )
    ...
    return {
        ...
        "binding_pairs_json": binding_pairs_json,
        "sentence_roles_json": sentence_roles_json,
...
def _render_case_markdown(row: dict[str, str]) -> list[str]:
    ...
    lines = [
        ...
        "",
        "**Sortable Material Text**",
        "",
        row["sortable_material_text"] or "(empty)",
        "",
        f"**Binding Pairs**: `{row['binding_pairs_json']}`",
        f"**Sentence Roles**: `{row['sentence_roles_json']}`",
        "",
        "**Anchor Stem**",
        "",
        f"> {row['anchor_stem']}",
