...
function Count-Matches {
    ...
    $count = 0
    foreach ($p in $Patterns) { $count += [regex]::Matches($Text, $p).Count }
    return $count
}
...
$degreeName = U @(0x7A0B,0x5EA6,0x8BCD)
$themeName = U @(0x4E3B,0x9898,0x8BCD)

$batchName = "center_understanding_standalone_subfamilies_$BatchDate"
...
$docxFiles = Get-ChildItem -LiteralPath $InputDir -Filter "*.docx" | Where-Object {
    ...
    ($base -eq $degreeName) -or ($base -eq $themeName)
} | Sort-Object Name
...
foreach ($f in $docxFiles) {
    ...
    $subfamilyId = if ($patternTag -eq $degreeName) { "degree_words" } else { "theme_words" }
    ...
    foreach ($blockRaw in $blocks) {
        ...
        $degreeMarkerCount = Count-Matches -Text $material -Patterns @(
            (U @(0x66F4)), (U @(0x6700)), (U @(0x6781,0x5176)), (U @(0x76F8,0x5BF9)),
            (U @(0x8F83,0x4E3A)), (U @(0x8D8A,0x6765,0x8D8A)), (U @(0x8FDB,0x4E00,0x6B65))
        )
        $themeMarkerCount = Count-Matches -Text $material -Patterns @(
            (U @(0x4E3B,0x9898)), (U @(0x4E3B,0x65E8)), (U @(0x4E2D,0x5FC3)),
            (U @(0x6838,0x5FC3)), (U @(0x5173,0x952E))
        )

        $rows.Add([PSCustomObject]@{
            family = $familyName
            subfamily = $patternTag
            ...
            degree_marker_count = $degreeMarkerCount
            theme_marker_count = $themeMarkerCount
        })
    }
}
...
$subStats = $allRows | Group-Object subfamily_id | Sort-Object Name | ForEach-Object {
    $g = $_.Group
    [PSCustomObject]@{
        subfamily_id = $_.Name
        ...
        avg_degree_markers = [math]::Round((($g | Measure-Object degree_marker_count -Average).Average), 2)
        avg_theme_markers = [math]::Round((($g | Measure-Object theme_marker_count -Average).Average), 2)
    }
}
...
$summary = [PSCustomObject]@{
    ...
    input_dir = $InputDir
    taxonomy_note = "degree_words and theme_words are standalone subfamilies under center_understanding, with no grandson families"
    total_samples = $total
    avg_material_len = $avgLen
    material_len_p25 = $p25
    material_len_p75 = $p75
    avg_sentence_count = $avgSent
    subfamily_stats = $subStats
}
$summary | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $summaryPath -Encoding utf8

$overview = @"
# Distill Pack Overview (center_understanding / standalone_subfamilies)

## Taxonomy
- mother_family: center_understanding
- standalone_subfamilies:
  - degree_words
  - theme_words
- no_grandson_families: true
...
$subGroups = $allRows | Group-Object subfamily_id
foreach ($sub in $subGroups) {
    $subName = $sub.Name
    $items = @($sub.Group)
    $subPath = Join-Path $outputDir ("distill_pack_" + $subName + ".md")
    $cnt = $items.Count
    ...
    $degAvg = if ($cnt -gt 0) { [math]::Round((($items | Measure-Object degree_marker_count -Average).Average), 2) } else { 0 }
    $thmAvg = if ($cnt -gt 0) { [math]::Round((($items | Measure-Object theme_marker_count -Average).Average), 2) } else { 0 }
    $docs = ($items | Group-Object source_doc | Sort-Object Name | ForEach-Object { $_.Name }) -join ", "

    $pack = @"
# Distill Pack: $subName
...
- avg_degree_markers: $degAvg
- avg_theme_markers: $thmAvg
...
- degree_marker_count / theme_marker_count
"@
    Set-Content -LiteralPath $subPath -Value $pack -Encoding utf8
}
